...
import com.secsbrain.frame.task.core.Version;
import com.secsbrain.frame.task.util.ZKTools;
...
public class ZKManager {
    ...
    private enum keys {
        zkConnectString, rootPath, userName, password, zkSessionTimeout, ipBlacklist
    }
    ...
    private void createZookeeper(final CountDownLatch connectionLatch) throws Exception {
        zk = new ZooKeeper(this.properties.getProperty(keys.zkConnectString.toString()),
                Integer.parseInt(this.properties.getProperty(keys.zkSessionTimeout.toString())), new Watcher() {
        ...
        if (zk.exists(this.getRootPath(), false) == null) {
            ...
            if (value == null) {
                ...
                if (!Version.isCompatible(dataVersion)) {
                    throw new Exception("Task Schedule程序版本 " + Version.getVersion() + " 不兼容Zookeeper中的数据版本 "
                            + dataVersion);
                }
        ...
        for (int i = 0; i < list.length - 1; i++) {
            ...
            if (StringUtils.isNotEmpty(str)) {
                ...
                if (zk.exists(zkPath, false) != null) {
                    ...
                    if (value != null
                            && new String(value).contains(Version.getVersion().substring(0,
                                    Version.getVersion().lastIndexOf("-") + 1))) {
                        throw new Exception(
                                "\""
                                        + zkPath
