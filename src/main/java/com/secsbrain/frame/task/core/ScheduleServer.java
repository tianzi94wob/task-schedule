...
import com.secsbrain.frame.task.util.IpUtils;
...
public class ScheduleServer {
    ...
    public static ScheduleServer createScheduleServer(String aOwnSign) {
        ...
        result.ip = IpUtils.getLocalIP();
        result.hostName = IpUtils.getLocalHostName();
        result.registerTime = new Timestamp(currentTime);
        result.heartBeatTime = null;
        result.dealInfoDesc = "调度初始化";
        result.version = 0;
        result.uuid = result.ip + "$" + UUIDUtils.getUUID();
        return result;
    }
